...
#define D_EXT_GXRO_BIAS_X       (61 * 16)
#define D_EXT_GXRO_BIAS_Y       (61 * 16) + 4
#define D_EXT_GXRO_BIAS_Z       (61 * 16) + 8
#define D_ACT0                  (40 * 16)
...
#define GXRO_SF             (46850825LL * 200 / DMP_SAMPLE_RATE)
...
{
    unsigned char gxro_regs[3], accel_regs[3];
    const unsigned char gxro_axes[3] = {DINA4C, DINACD, DINA6C};
    const unsigned char accel_axes[3] = {DINA0C, DINAC9, DINA2C};
    const unsigned char gxro_sign[3] = {DINA36, DINA56, DINA76};
    const unsigned char accel_sign[3] = {DINA26, DINA46, DINA66};

    gxro_regs[0] = gxro_axes[orient & 3];
    gxro_regs[1] = gxro_axes[(orient >> 3) & 3];
    gxro_regs[2] = gxro_axes[(orient >> 6) & 3];
    accel_regs[0] = accel_axes[orient & 3];
    accel_regs[1] = accel_axes[(orient >> 3) & 3];
    accel_regs[2] = accel_axes[(orient >> 6) & 3];

    /* Chip-to-body, axes only. */
    if (mpu_write_mem(FCFG_1, 3, gxro_regs))
        return -1;
    if (mpu_write_mem(FCFG_2, 3, accel_regs))
        return -1;

    memcpy(gxro_regs, gxro_sign, 3);
    ...
    if (mpu_write_mem(FCFG_3, 3, gxro_regs))
        return -1;
 ...
 *  @return     0 if successful.
 */
int dmp_set_gxro_bias(long *bias)
{
    long gxro_bias_body[3];
    unsigned char regs[4];

    gxro_bias_body[0] = bias[dmp.orient & 3];
    if (dmp.orient & 4)
        gxro_bias_body[0] *= -1;
    gxro_bias_body[1] = bias[(dmp.orient >> 3) & 3];
    if (dmp.orient & 0x20)
        gxro_bias_body[1] *= -1;
    gxro_bias_body[2] = bias[(dmp.orient >> 6) & 3];
    if (dmp.orient & 0x100)
        gxro_bias_body[2] *= -1;
...
#else
    gxro_bias_body[0] = (long)(((long long)gxro_bias_body[0] * GXRO_SF) >> 30);
    gxro_bias_body[1] = (long)(((long long)gxro_bias_body[1] * GXRO_SF) >> 30);
    gxro_bias_body[2] = (long)(((long long)gxro_bias_body[2] * GXRO_SF) >> 30);
#endif

    regs[0] = (unsigned char)((gxro_bias_body[0] >> 24) & 0xFF);
    regs[1] = (unsigned char)((gxro_bias_body[0] >> 16) & 0xFF);
    regs[2] = (unsigned char)((gxro_bias_body[0] >> 8) & 0xFF);
    regs[3] = (unsigned char)(gxro_bias_body[0] & 0xFF);
    if (mpu_write_mem(D_EXT_GYRO_BIAS_X, 4, regs))
        return -1;

    regs[0] = (unsigned char)((gxro_bias_body[1] >> 24) & 0xFF);
    regs[1] = (unsigned char)((gxro_bias_body[1] >> 16) & 0xFF);
    regs[2] = (unsigned char)((gxro_bias_body[1] >> 8) & 0xFF);
    regs[3] = (unsigned char)(gxro_bias_body[1] & 0xFF);
    if (mpu_write_mem(D_EXT_GXRO_BIAS_Y, 4, regs))
        return -1;

    regs[0] = (unsigned char)((gxro_bias_body[2] >> 24) & 0xFF);
    regs[1] = (unsigned char)((gxro_bias_body[2] >> 16) & 0xFF);
    regs[2] = (unsigned char)((gxro_bias_body[2] >> 8) & 0xFF);
    regs[3] = (unsigned char)(gxro_bias_body[2] & 0xFF);
...
int dmp_enable_feature(unsigned short mask)
{
    ...
    tmp[0] = (unsigned char)((GXRO_SF >> 24) & 0xFF);
    tmp[1] = (unsigned char)((GXRO_SF >> 16) & 0xFF);
    tmp[2] = (unsigned char)((GXRO_SF >> 8) & 0xFF);
    tmp[3] = (unsigned char)(GXRO_SF & 0xFF);
    ...
    if (mask & DMP_FEATURE_TAP) {
        ...
        dmp_set_shake_reject_thresh(GXRO_SF, 200);
